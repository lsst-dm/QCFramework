...
######################### PATTERN CONFIG #######################
my $patternId = getnextId('qc_threshold',$desdbh);
...
my $sqlPattern = 'insert into qc_pattern values ('.$patternHash->{'id'}.','.$patternHash->{'pattern'}.','.$patternHash->{'valid'}.','.$patternHash->{'timestamp'}.','.$patternHash->{'type'}.','.$patternHash->{'exec_id'}.')';
